...
namespace Aufgabe6_Bienen {
    ...
    function init(_event: Event): void {
        ...
        for (let i: number = 0; i < n; i++) {
            let bee: any = {x: 0, y: 0, size: 0, color: "", direction: 0};
            beeArr[i] = bee;
            bee.x = 250;
            bee.y = 620;
            bee.color = "#" + Math.floor(Math.random() * 16777215).toString(16);
            bee.direction = (Math.random() * 1.5);
    ...
    function animate(): void {
        ...
        for (let i: number = 0; i < beeArr.length; i++) {
            let bee = beeArr[i];
            bee.x += Math.random() * 3 - 2 + bee.direction;
            bee.y += Math.random() * 4 - 2;
     ...
     function drawBee(_x: number, _y: number, _color: any): void {
        crc2.beginPath();
        crc2.moveTo(_x, _y);
